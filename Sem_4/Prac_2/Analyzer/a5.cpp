...
class Lexer
{
    ...
    void set_flag_str() { flag_rd = 10; }
    void set_flag_file() { flag_rd = 1; }
    bool is_flag_str() { return flag_rd == 10; }
    bool is_flag_file() { return flag_rd == 1; }
...
public:
    ...
    ~Lexer() { if(is_flag_file()) fclose(fd); }
...
Lex Lexer::get_lex()
{
    ...
    while (true)
    {
        ...
        {
        case H:
            if((is_flag_file() && c == EOF) || (is_flag_str() && index >= int(prog.length())))
                G = FIN_OF_FILE;
            else if ((c == '\n' || c == '\r' || c == ' ' || c == '\t'))
                get_symb();
            else if (isalpha(c))
        ...
        case COM:
            ...
            else if ((is_flag_file() && c != EOF) || (is_flag_str() && index <= int(prog.length())))
                get_symb();
            else
                G = FIN_OF_FILE;
            break;
...
public:
    ...
    Parser(std::string str) : scan(str), prog(1000) {}
    void analyze();
};
...
public:
    ...
    Interpretator(std::string str) : pars(str){};
    void interpretation();
};
...
int main(int argc, char **argv)
{
    try
    {
        ...
        else
        {
            ...
            /*std::string str = " program{ int t = 10; if(t < 5) write(1); else write(2); } ";
            //std::cout<<str.length();
            Interpretator I(str);
            I.interpretation();*/
        }
